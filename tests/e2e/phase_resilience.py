...
from harness import (Client, OWNER_EMAIL, OWNER_PASSWORD, Proc, SERVICE_ID,
...
try:
    ...
    owner.sign_up_owner()
    phone = Client(TUNNEL)
    phone.cookie = SESSION + "; " + owner.cookie
    device_token = owner.pair_a_laptop("resilience laptop")
    now = int(time.time())
    ...
    async def drain(sp):
        fw = Forwarder(sp, url=f"{TUNNEL}/ingest", token=device_token, batch_size=50,
    ...
    status, dev = phone.call(f"/api/device")
    ...
    status, _ = jget(f"{TUNNEL}/status", method="POST", data=json.dumps(beat).encode(),
                     headers={**SVC, **JSON_H, "Authorization": f"Bearer {device_token}"})
    ...
    jget(f"{TUNNEL}/status", method="POST", data=json.dumps(stale).encode(),
         headers={**SVC, **JSON_H, "Authorization": f"Bearer {device_token}"})
    status, dev = owner.call(f"/api/device")
    ...
    status, _ = owner.call(f"/api/journal/{TODAY}", method="PUT",
                           data={"tags": ["nap"], "amounts": {},
                                 "notes": "written on the laptop"})
    ...
    status, entry = phone.call(f"/api/journal/{TODAY}")
    ...
    def writer(label, client, n):
        for i in range(n):
            st, _ = client.call(f"/api/journal/{TODAY}", method="PUT",
                                data={"tags": [label], "amounts": {},
                                      "notes": f"{label} {i}"})
            (results if st == 200 else errors).append((label, st))

    threads = [threading.Thread(target=writer, args=("laptop", owner, 25)),
               threading.Thread(target=writer, args=("phone", phone, 25))]
    ...
    def stream():
        ...
        while not stop.is_set() or batches < 25:
            ...
            st, body = jget(f"{LAN}/ingest", method="POST",
                            ...
                            headers={**JSON_H, "Authorization": f"Bearer {device_token}"})
    ...
    write_fail = []
    for i in range(20):
        st, _ = phone.call("/api/intake", method="POST",
                           data={"at": f"{TODAY}T{9 + i % 12:02d}:00:00Z",
                                 "substance": "caffeine", "amount": 80,
                                 "label": "coffee"})
        if st != 200: write_fail.append(st)
        st, _ = phone.call(f"/api/journal/{TODAY}", method="PUT",
                           data={"tags": ["caffeine"], "amounts": {},
                                 "notes": f"note {i}"})
        if st != 200: write_fail.append(st)
